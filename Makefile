...
# Host tools are provided on PATH by the build image
# (laqieer/gba-disasm-decomp-tools:fireemblem8u), not vendored under tools/.
CC1        := agbcc$(EXE)
...
AIF2PCM    := aif2pcm$(EXE)
MID2AGB    := mid2agb$(EXE)
TEXTENCODE := textencode$(EXE)
JSONPROC   := jsonproc$(EXE)
PREPROC    := preproc$(EXE)
...
AGBCC_LIB ?= /usr/local/bin/agbcc/lib
# CP932 conversion: the Alpine image provides GNU libiconv as `gnu-iconv`.
ICONV ?= gnu-iconv
...
$(ELF): $(ALL_OBJECTS) $(OBJECTS_LST) $(LDSCRIPT) $(SYM_FILES)
	$(LD) -T $(LDSCRIPT) -Map $(MAP) @$(OBJECTS_LST) -R $(BANIM_OBJECT).sym.o -L $(AGBCC_LIB) -o $@ -lc -lgcc
...
# preproc is provided on PATH by the build image (not vendored), so it is invoked
# as a command rather than built/required as a prerequisite.
$(DATA_SRC_C_OBJECTS): %.o: %.c $$(data_dep)
