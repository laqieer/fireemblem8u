...
- 转换链：`gbagfx foo.gbapal foo.pal` → 美术编辑 → `gbagfx foo.pal foo.gbapal`（统一使用官方 `gbagfx`，配合 `.gitattributes` 将 `.pal` 在检出时强制成 CRLF，避免跨平台差异）。
...
## 资源提取·解压·回填：经验与注意事项（2026-02-28 更新）

### 1. 作业前 Checklist
- **启动文件**：每次上下文重置都要先确认 `WORKFLOW_AUTO.md`（若缺席需记录 ENOENT）并重读当日 `memory/YYYY-MM-DD.md`，否则后续结论不算数。
- **Git 状态**：`banim-assets-source` 必须保持“相对 `origin/master` 仅文档变化”，因此任何资产回填都要在同目录完成并随时 `git status` 核对改动范围。
- **工具链**：`tools/gbagfx/gbagfx` 是唯一允许的压缩/解压工具；`scripts/jasc_pal_to_gbapal.py` 仅作为历史记录，实际转换一律调用 gbagfx，并确保 `.pal` 以 CRLF 存入。

### 2. 标准流程
1. **定位资产**：借助 `reports/data_banim_incbin_labels.csv`/`reports/data_banim_asset_map.csv` 锁定 `.incbin` 的 ROM 偏移与尺寸，必要时用 `xxd`/`hexdump` 快速确认结构开头（`0x10`=LZ77、TSA 高位=palette 位等）。
2. **切片&命名**：按 `<ROMOFFSET>_<Label>` 命名到 `graphics/banim/assets/{img,pal,tsa,animscr,misc}`，未明结构先放 `misc/` 并立刻记录来源。
3. **解压与可读化**：
   - `.4bpp.lz` → `.4bpp` → `.png`（使用 `-num_tiles` / `-width`，缺省时优先在 `banim_img_rules.mk` 里补规则后再回写）。
   - `.tsa.lz` → `.tsa`，若判定为无头 TSA，使用 PNG + 脚本提供的列数验证 tile 总数；必要时补注释说明（例如 32×16 背景底板）。
   - `.gbapal` → `.pal`（CRLF），如检测到 MSB=1 则保留 `.agbpal` 形式直接引用。
4. **回填数据段**：`.incbin` 应尽量引用仓库文件；若必须混合 ROM 残段（例如压缩后变短），务必写成“资产文件 + `baserom` 剩余切片”并计算好偏移/长度，直到确认残段也已被拆出。
5. **构建验证**：统一跑 `make clean && make -j8 > build.log 2>&1` → `sha1sum -c checksum.sha1` → `scripts/bindiff.sh baserom.gba fireemblem8.gba`，失败就按“修复 → 重新执行命令”循环。
6. **文档与日志**：
   - `docs/banim_asset_extraction.md` 记录方法、命名依据、edge case；
   - `docs/banim_asset_process_log.md` 逐条抄写 Feishu 指令与响应；
   - `memory/YYYY-MM-DD.md` 写 durable checkpoint，尤其是 compaction 前。
7. **提交规范**：commit message 必须附 `Agent: <name> | Model: github-copilot/gpt-5.1-codex | OpenClaw: 2026.2.25` trailer；推送前再次确认 ROM parity。

### 3. 常见坑位
- **PNG/源文件被清理**：`make clean` 会删掉 `.4bpp/.tsa.lz` 等构建产物，但 PNG/源 TSA 不应该消失。若误删，需从 ROM 重切并及时 commit，避免下一次构建报 “Failed to open …png”。
- **残余切片**：当 `.incbin` 被拆成“压缩产物 + ROM 尾段”时，不要长期留着裸 `baserom` 引用。应尽快识别尾段（可能是 TSA、调色板或脚本），生成具名资产后删掉残片。
- **gbapal ↔ pal 往返**：转换后务必 check MSB，若 `bit15` 被清零会导致 ROM 与 GitHub 下载的 `.pal` 不一致——这一类直接改用 `.agbpal` 并注明原因。
- **宽度/tiles 参数**：缺少规则会使 `gbagfx` 默认 32 列，导致重新压缩后长度变化而触发指针漂移。新资产记得更新 `banim_img_rules.mk` 或至少写出 `-num_tiles`。
- **构建命令**：当 Feishu 要求“make clean && make”时，不得用 `make -j8` 替代；必须严格按指令顺序运行并提供日志/校验结果。

### 4. 对外同步模板
- **状态**：提及解析对象、命名、构建/校验结果以及提交 SHA。
- **风险**：若存在未拆的残段或缺 PNG，优先说明影响（例如 ROM 仍依赖 `baserom` 切片）。
- **下一步**：列出剩余任务（比如“继续拆 `0x678D20+0x584` 残段”、“整理 TSA 宽度规则”），保持透明度。

### 2026-02-28 15:20 UTC 补充：校验与回填细则
1. **LZ 流再压缩对比**：任何 `.lz` 在 `gbagfx -d` 解压之后，必须再用 `gbagfx` 重新压回 `.lz` 并与 ROM 原片段 byte-to-byte 比对，确认后端没有隐藏未命名资源。若发现剩余字节（尺寸不一致），要立即标注并单独提取这些残段，避免“压缩段 + baserom 切片”长期共存。
2. **4bpp ↔ PNG 往返校验**：
   - `.4bpp.lz` → `.4bpp` → `.png` 后，务必 `gbagfx PNG→4bpp` 重建并与原 `.4bpp` 对比，确认 tile 数/排列完全一致。
   - 如果对不上，先看配套 TSA 是否带头部：能读出宽高就按头部尺寸梳理 tile 数；若 TSA 无头或仍无法整除，就直接在 `gbagfx` 调用里指定 `-num_tiles` 或 `-width`，使重新生成的 `.4bpp` 与 ROM 对齐。
3. **调色板高位规则**：
   - 批量检查 `.gbapal` 中每个 RGB555 颜色的最高位（bit15）。如果全部为 0，可用 `gbagfx` 转成 `.pal`（注意 `.pal` 必须是 CRLF 换行，`gbagfx` 不接受 LF），再转回 `.gbapal` 并与原文件对比，确认 round-trip 无损。
   - 只要检测到某个颜色 bit15=1，就不要做 `.pal` 往返，直接把文件后缀改成 `.agbpal` 并在 `.incbin` 中引用该 `.agbpal`，因为 `gbagfx` 在 `pal→gbapal` 时会把最高位全部置 0，无法还原 ROM 数据。

> 上述校验是“提取→回填”环节的硬性步骤，任何差分都必须在文档/日志中记录原因和处理方案。

> **常驻指令（2026-02-28 15:38 UTC）**：本仓库所有任务在提交/推送前必须先本地校验——构建命令固定为 `make clean && make`，需要暂停其他工作、执行命令、若失败则先修复后重复该命令直到成功；提交时，commit message 末尾必须追加 `Agent: <name> | Model: github-copilot/gpt-5.1-codex | OpenClaw: 2026.2.25`。此规范已写入文档并视为永久要求。
